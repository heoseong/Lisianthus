...
library(lares)
h2o.init()
lisianthus <- h2o.importFile(path = "SPAD.csv")
h2o.dim(lisianthus)
h2o.ls()
...
model_names[1]
top_model <- h2o.getModel("StackedEnsemble_BestOfFamily_7_AutoML_1_20230717_175022")
...
ggplot(data=real_frame, aes(x=Vase_life, y=h2o_predictions$predict, color=Cultivar, fill = Cultivar)) +
  ...
  annotate(geom="text", x=10, y=8, label = "StackedEnsemble_BestOfFamily_4_AutoML_1", hjust=0) +
  annotate(geom="text", x=10, y=7.5, label = "RMSE: 1.1654", hjust=0) +
  annotate(geom="text", x=10, y=7, label = "MAE: 0.8155", hjust=0) +  
  annotate(geom="text", x=10, y=6.5, label = expression(R^2: ~ 0.6966), hjust=0)
